...
function Login() {
  ...
  function validateForm(): boolean {
    ...
    if (!emailRegex.test(form.usuario)) newErrors.usuario = 'Informe um e-mail válido.';
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      await login({ usuario: form.usuario, senha: form.senha });
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <div className="flex flex-col space-y-1">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm focus:outline-none focus:ring-2 transition-all placeholder-slate-400 ${
                  errors.usuario ? 'border-red-400 focus:ring-red-300' : 'border-slate-200 focus:border-[#14B8A6] focus:ring-[#14B8A6]/20'
                }`}
              />
            ...
            <div className="flex flex-col space-y-1">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-sm focus:outline-none focus:ring-2 transition-all placeholder-slate-400 ${
                  errors.senha ? 'border-red-400 focus:ring-red-300' : 'border-slate-200 focus:border-[#14B8A6] focus:ring-[#14B8A6]/20'
                }`}
              />
            ...
            <div className="flex items-center justify-between pt-4">
              <p className="text-sm text-slate-400">
                Não tem conta?{' '}
                <Link to="/cadastro" className="font-bold transition-colors" style={{ color: '#14B8A6' }}>
                  Cadastre-se
                </Link>
              </p>
              <button
